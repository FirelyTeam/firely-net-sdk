...
using Hl7.Fhir.Rest.Search;

namespace Hl7.Fhir.Test.Rest
{
    [TestClass]
	public class NewSearchParamsTests
    {
        [TestMethod]
        ...
        public void ParseAndSerializeParams()
        {
            ...
            var q2 = SearchParams.FromUriParamList(Fhir.Rest.UriParamList.FromQueryString(output));
        ...
        [TestMethod]
        public void TestConstructParams()
        {
            var o = All.Query.Is("myQuery");
            Assert.AreEqual("_query=myQuery", o.ToString());

            var p = All.Count.GreaterThan(4).Or(All.Count.EqualTo(5));
            Assert.AreEqual("_count=gt4,eq5", p.ToString());

            var q = All.Demo.Values(All.Count.Is(4), All.Query.Is("xxx"));
            Assert.AreEqual("_demo=_count$4,_query$xxx", q.ToString());

            var r = All.Count.Missing();
            Assert.AreEqual("_count:missing=true", r.ToString());

            var s = All.Summary.Is(SummaryType.Text);
            Assert.AreEqual("_summary=text", s.ToString());

            var then = new DateTimeOffset(2017, 1, 1, 0, 0, 0, TimeSpan.Zero);
            var t = All.Birthdate.StartsAfter("2016-01-01").Or(All.Birthdate.Is(then));
            Assert.AreEqual("birthdate=sa2016-01-01,2017-01-01T00:00:00+00:00", t.ToString());

            var u = All.Name.Contains("wout");
            Assert.AreEqual("name:contains=wout", u.ToString());

            var v = All.Kind.Is("http://nu.nl", "bla");
            Assert.AreEqual("kind=http://nu.nl|bla", v.ToString());
            v = All.Kind.Is(null, "bla");
            Assert.AreEqual("kind=|bla", v.ToString());
            v = All.Kind.Above(new Coding { System = "http://nu.nl", Code = "bla" });
            Assert.AreEqual("kind:above=http://nu.nl|bla", v.ToString());
        }
