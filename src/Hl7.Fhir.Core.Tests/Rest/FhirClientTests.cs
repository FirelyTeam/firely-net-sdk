...
namespace Hl7.Fhir.Tests.Rest
{
    ...
    public class FhirClientTests
    {
        ...
        {
            var client = new FhirClient(testEndpoint);        // an address that returns html
        ...
        {
            using (var client = new FhirHttpClient(testEndpoint))        // an address that returns html
            {
                try
                {
                    var pat = client.Read<Patient>("Patient/pat1r4");
        ...
        {
            using (var client = new FhirHttpClient(testEndpoint))// an address that returns Status 404 with an OperationOutcome
            {
                testHandlingErrorStatusAsOperationOutcome(client);
            }
        }
