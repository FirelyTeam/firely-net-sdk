...
public class FhirPathContextTests
{
    ...
    public void SetupSource()
    {
        ...
        _bundle = (new FhirXmlParser()).Parse<Bundle>(bundleXml).ToElementNode();
    }
    ...
    public void TestFhirEvaluationContext()
    {
        ...
        var elemInContainedResource = _bundle.Child("entry").Skip(2).First().Child("resource").First().Child("contained").First().Child("id").First().ToScopedNode();
        ...
        var elemInDomainResource = _bundle.Child("entry").Skip(2).First().Child("resource").First().Child("id").First().ToScopedNode();
