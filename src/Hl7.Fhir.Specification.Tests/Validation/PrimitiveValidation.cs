...
namespace Hl7.Fhir.Validation
{
    ...
    public class PrimitiveValidation
    {
        ...
        {
            var def = source.FindStructureDefinitionForCoreType(FHIRAllTypes.Oid);
        ...
        {
            var extensionSD = (StructureDefinition)source.FindStructureDefinitionForCoreType(FHIRAllTypes.Extension).DeepCopy();
            ...
            extValueDef.Type.RemoveAll(t => t.Code == FHIRAllTypes.Oid.GetLiteral());
        ...
        {
            var patientSD = (StructureDefinition)source.FindStructureDefinitionForCoreType(FHIRAllTypes.Patient);
        ...
        public void ValidateOverNameRef()
        {
            ...
            var questionnaireSD = source.FindStructureDefinitionForCoreType(FHIRAllTypes.Questionnaire);
        ...
        public void ValidateContained()
        {
            ...
            var careplanSD = source.FindStructureDefinitionForCoreType(FHIRAllTypes.CarePlan);

            var report = validator.Validate(careplanSD, careplan);
            Assert.IsTrue(report.Success);
            Assert.AreEqual(3, report.Warnings);            // 4x missing xhtml
        }
