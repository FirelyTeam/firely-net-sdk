...
namespace Hl7.Fhir.Specification.Tests
{
    ...
    public class OutcomeReportingTest
    {
        ...
        public void Setup()
        {
            ...
            level0.AddIssue(Support.Issue.CONTENT_ELEMENT_HAS_INCORRECT_TYPE.ToIssueComponent("A test error at level 0", "Patient"));
            ...
            level2.AddIssue(Support.Issue.UNSUPPORTED_CONSTRAINT_WITHOUT_FHIRPATH.ToIssueComponent("A test warning at level 2", "Patient.active[0].id[0]"));
            level2.AddIssue(Support.Issue.CONTENT_ELEMENT_MUST_MATCH_TYPE.ToIssueComponent("Another test error at level 2", "Patient.active[0].id[0]"));

            level1.Include(level2);

            level0.Include(level1);

            level0.AddIssue(Support.Issue.PROFILE_ELEMENTDEF_IS_EMPTY.ToIssueComponent("A test warning at level 0", "Patient"));
        ...
        public void IssueCategorization()
        {
            ...
            Assert.AreEqual(3, _report.Where(severity: IssueSeverity.Warning).Count());
            Assert.AreEqual(2, _report.Where(type: IssueType.BusinessRule).Count());
            Assert.AreEqual(1, _report.Where(issueCode: Support.Issue.PROFILE_ELEMENTDEF_CARDINALITY_MISSING.Code).Count());
            Assert.AreEqual(1, _report.Where(severity: IssueSeverity.Warning, type: IssueType.BusinessRule, issueCode: 2008).Count());
            Assert.AreEqual(0, _report.Where(severity: IssueSeverity.Error, type: IssueType.BusinessRule, issueCode: 2008).Count());
        }
