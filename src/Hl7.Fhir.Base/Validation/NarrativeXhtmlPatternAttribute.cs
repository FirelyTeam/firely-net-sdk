            ...
            if (value is string xml)
            {
                return kind switch
                {
                    ...
                    NarrativeValidationKind.Xml => XHtml.IsValidXml(xml, out var error)
                            ...
                            : COVE.NARRATIVE_XML_IS_MALFORMED(context, error).AsResult(),
...
#endif
                return errors.Any() ? COVE.NARRATIVE_XML_IS_INVALID(context, string.Join(", ", errors)).AsResult() : ValidationResult.Success!;
            }
            catch (FormatException fe)
            {
                return COVE.NARRATIVE_XML_IS_MALFORMED(context, fe.Message).AsResult();
            }
        }
    }
}
