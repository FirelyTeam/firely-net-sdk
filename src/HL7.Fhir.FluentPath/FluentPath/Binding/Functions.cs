...
namespace HL7.Fhir.FluentPath.FluentPath.Binding
{
    ...
    {
        public delegate object Builtin(IEnumerable<IFluentPathValue> focus);
        public delegate object Builtin<A>(IEnumerable<IFluentPathValue> focus, A argument1);
        public delegate object Builtin<A, B>(IEnumerable<IFluentPathValue> focus, A argument1, B argument2);
        ...
        private static IEnumerable<IFluentPathValue> castResult(object result)
        {
            ...
            else if (result is IFluentPathValue)
            {
                if (result == null)
                    ...
                    return FhirValueList.Create((IFluentPathValue)result);
            }
            else
