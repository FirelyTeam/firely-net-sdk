...
namespace Hl7.Fhir.Model
{
    public static class ElementDefinitionExtensions
    {
        ...
        public static string? GetTypeProfile(this ElementDefinition.TypeRefComponent elemType) =>
            elemType?.Profile.SafeSingleOrDefault() ?? (elemType?.Code is not null ? CommonModelInfo.CommonCanonicalUriForFhirCoreType(elemType.Code).Value : null);
        ...
        public static IEnumerable<string>? GetTypeProfiles(this ElementDefinition.TypeRefComponent elemType) =>
            elemType?.Profile.Any() == true ? elemType.Profile : (elemType?.Code is not null ? new[] { CommonModelInfo.CommonCanonicalUriForFhirCoreType(elemType.Code).Value } : null);
