...
namespace Hl7.Fhir.Serialization
{
    ...
    {
        public PocoBuilder(Model.Version version)
        {
            _settings = new PocoBuilderSettings(version);
        }

        public PocoBuilder(PocoBuilderSettings settings)
        {
        ...
        public Base BuildFrom(ISourceNode source, Type dataType = null)
        {
            ...
            if (dataType != null)
            {
                typeFound = AllVersionsModelInfo.GetFhirTypeNameForType(dataType);
        ...
        public Base BuildFrom(ISourceNode source, string dataType = null)
        {
            ...
            var typedSource = source.ToTypedElement(new PocoStructureDefinitionSummaryProvider(_settings.Version), dataType, typedSettings);
        ...
        public Base BuildFrom(ITypedElement source)
        {
            ...
            Base build()
            {
                var typeToBuild = ModelInfos.Get(_settings.Version).GetTypeForFhirType(source.InstanceType);
                ...
                var settings = new ParserSettings(_settings.Version)
                {
