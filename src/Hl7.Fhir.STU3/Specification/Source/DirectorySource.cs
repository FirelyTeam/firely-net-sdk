...
public class DirectorySource : ISummarySource, IConformanceSource, IArtifactSource, IAsyncResourceResolver
{
    ...
    private DirectorySource(string contentDirectory, DirectorySourceSettings settings, bool cloneSettings)
    {
        ...
        _navigatorFactoryDelegate = _navigatorFactory.Create;
    ...
    public Stream LoadArtifactByName(string filePath)
    {
        ...
        bool isMatch(ArtifactSummary summary)
            => PathComparer.Equals(summary.Origin, filePath)
               || summary.Origin?.EndsWith(Path.DirectorySeparatorChar + filePath, PathComparison) is true;

        // Only consider valid summaries for recognized artifacts
        bool isCandidateArtifact(ArtifactSummary summary)
            => summary?.Origin != null && isMatch(summary);
    ...
    private T loadResourceInternal<T>(ArtifactSummary summary) where T : Resource
    {
        ...
        if (navStream == null || !navStream.Seek(pos)) return null;
        ...
        T result = nav.ToPoco<T>(pocoSettings);
    ...
    {
        _settings.XmlParserSettings?.CopyTo(_navigatorFactory.XmlParsingSettings);
        _settings.JsonParserSettings?.CopyTo(_navigatorFactory.JsonParsingSettings);
        return _navigatorFactory;
    }
