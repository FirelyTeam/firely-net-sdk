...
namespace Hl7.Fhir.Specification.Tests
{
    ...
    public class SummarySourceTests
    {
        ...
        public void ListSummaries()
        {
            ...
            var de = source.ListSummaries(ResourceType.DataElement); Assert.IsEmpty(de);
            ...
            var sp = source.ListSummaries(ResourceType.SearchParameter); Assert.IsEmpty(sp);
            ...
            var cs = source.ListSummaries(ResourceType.CodeSystem); Assert.IsEmpty(cs);
            var vs = source.ListSummaries(ResourceType.ValueSet); Assert.IsNotEmpty(vs);
            var cm = source.ListSummaries(ResourceType.ConceptMap); Assert.IsEmpty(cm);
            var ep = source.ListSummaries(ResourceType.ExpansionProfile); Assert.IsEmpty(ep);
            var ns = source.ListSummaries(ResourceType.NamingSystem); Assert.IsEmpty(ns);
