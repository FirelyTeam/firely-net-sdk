...
namespace Hl7.Fhir.Serialization.Tests
{
    ...
    public class ParseDemoPatientJsonTyped
    {
        public ITypedElement getJsonNode(string json, FhirJsonParsingSettings settings = null) 
            => JsonParsingHelpers.ParseToTypedElement(json, new PocoStructureDefinitionSummaryProvider(Model.Version.DSTU2), settings: settings);
        ...
        public void PingpongJson()
        {
            ...
            var navJson = JsonParsingHelpers.ParseToTypedElement(tp, new PocoStructureDefinitionSummaryProvider(Model.Version.DSTU2));
            var xml = navJson.ToXml();

            var navXml = XmlParsingHelpers.ParseToTypedElement(xml, new PocoStructureDefinitionSummaryProvider(Model.Version.DSTU2));
        ...
        public void IgnoreElements()
        {
            ...
            var jsonBare = patient.ToTypedElement(new PocoStructureDefinitionSummaryProvider(Model.Version.DSTU2)).ToJson(new FhirJsonSerializationSettings { IgnoreUnknownElements = false });
            Assert.IsTrue(jsonBare.Contains("pat1"));

            patient.Add(SourceNode.Valued("unknownElement", "someValue"));
            var jsonUnknown = patient.ToTypedElement(new PocoStructureDefinitionSummaryProvider(Model.Version.DSTU2), settings: new TypedElementSettings { ErrorMode = TypedElementSettings.TypeErrorMode.Ignore }).ToJson(new FhirJsonSerializationSettings { IgnoreUnknownElements = true });
        ...
        public void CatchesComplexPrimitiveMismatch()
        {
            ...
            navJson = JsonParsingHelpers.ParseToTypedElement(tp, new PocoStructureDefinitionSummaryProvider(Model.Version.DSTU2));
        ...
        public void CatchesSingleValueForArray()
        {
            ...
            var navJson = JsonParsingHelpers.ParseToTypedElement(tp, new PocoStructureDefinitionSummaryProvider(Model.Version.DSTU2), null, new FhirJsonParsingSettings() { PermissiveParsing = false });
            var errors = navJson.VisitAndCatch();
            ...
            navJson = JsonParsingHelpers.ParseToTypedElement(tp, new PocoStructureDefinitionSummaryProvider(Model.Version.DSTU2), null, new FhirJsonParsingSettings() { PermissiveParsing = false });
            errors = navJson.VisitAndCatch();
