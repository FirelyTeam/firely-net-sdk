...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2">
  ...
  <sch:pattern>
    ...
    <sch:rule context="f:Parameters//f:parameter">
      <sch:assert test="(exists(f:part) and not(exists(f:*[starts-with(local-name(.), 'value')])) and not(exists(f:resource))) or (not(exists(f:part)) and (exists(f:*[starts-with(local-name(.),'value')]) != exists(f:resource)))">inv-1: A parameter must have one and only one of (value, resource, part)</sch:assert>
    </sch:rule>
  </sch:pattern>
</sch:schema>
