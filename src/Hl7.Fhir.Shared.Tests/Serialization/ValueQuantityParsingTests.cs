...
namespace Hl7.Fhir.Tests.Serialization
{
    ...
    public class ValueQuantityParsingTests
    {
        ...
        async static Tasks.Task<T> XmlRoundTripAsync<T>(T resource) where T : Resource
        {
            ...
            var parsed = new FhirXmlParser(new DeserializerSettings().UsingMode(DeserializationMode.Recoverable)).Parse<T>(xml);
        ...
        static async Tasks.Task<T> JsonRoundTrip<T>(T resource) where T : Resource
        {
            ...
            var parsed = new FhirJsonParser(new DeserializerSettings().UsingMode(DeserializationMode.Recoverable)).Parse<T>(json);
