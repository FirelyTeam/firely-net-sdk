...
namespace Hl7.Fhir.Navigation
{
    ...
    public interface IValueNavigationTree<T> : INavigationTree<T> where T : IValueNavigationTree<T>
    {
    }
    ...
    public interface IValueNavigationTree<T, V> : IValueNavigationTree<T>, IValueTreeBuilder<T, V>, IValueProvider<V> where T : IValueNavigationTree<T, V>
    {
    }
    ...
    public abstract class ValueNavigationTree<T, V> : NavigationTree<T>, IValueNavigationTree<T, V>, IMutableValueProvider<V> where T : ValueNavigationTree<T, V>
    {
...
#region ILinkedUniformValueTreeBuilder<T>
        ...
        public T AddFirstChild(string name, V value)
        {
            return AddFirstChild(() => CreateNode(Self, null, name, value));
        }
        ...
        /// <param name = "value">The node value.</param>
        /// <returns>A reference to the new node of type <typeparamref name = "T"/>.</returns>
        public T AddNextSibling(string name, V value)
        {
            return AddNextSibling(() => CreateNode(Parent, Self, name, value));
        }
